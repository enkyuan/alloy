...
def use_mock_provider():
    with patch(
        "agentkit_serve.runtime.nodes.agentic.get_provider",
...
async def test_node_text_response_no_tools(use_mock_provider):
    ...
    with patch(
        "agentkit_serve.runtime.nodes.agentic.build_tools_payload", return_value=[]
    ):
...
async def test_node_tool_loop_no_infra(use_mock_provider, registered_tool):
    ...
    with (
        patch(
            "agentkit_serve.runtime.nodes.agentic.execute_tool",
            side_effect=fake_execute_tool,
        ),
        patch(
            "agentkit.runtime.tools.retriever.ToolRetriever.get_top_tools",
            return_value=["ping"],
        ),
    ):
        node = AgentReasoningNode(system_prompt="You are helpful.")
