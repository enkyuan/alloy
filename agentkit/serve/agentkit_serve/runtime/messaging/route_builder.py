...
from agentkit_serve.runtime.messaging.bus import Message
from agentkit.modalities.voice.event_registry import EventType, EventTypeOrAlias
from agentkit.modalities.voice.utils.async_tasks import await_tasks_safe
...
class RouteConfig:
    ...
    source: Optional[Union[str, Callable[[str], bool]]] = None
    id_filter: Optional[Union[str, Callable[[str], bool]]] = None
    timestamp_filter: Optional[Union[float, Callable[[float], bool]]] = None
    max_concurrent_tasks: Optional[int] = None

    interrupt_handlers: Dict[EventTypeOrAlias, Optional[_EventHandlerDict]] = field(
        default_factory=dict
    )
    suspend_handlers: Dict[EventTypeOrAlias, Optional[_EventHandlerDict]] = field(
        default_factory=dict
    )
    resume_handlers: Dict[EventTypeOrAlias, Optional[_EventHandlerDict]] = field(
        default_factory=dict
    )

    # These filters operate on the message object.
    event_property_filters: Dict[str, Union[Any, Callable[[Any], bool]]] = field(
        default_factory=dict
    )
...
class RouteBuilder:
    ...
    def _has_control_operation(self) -> bool:
        ...
        return (
            len(self.route_config.operations) > 0
            and self.route_config.operations[0]["_fn_type"] == "control"
        )
    ...
    def _validate_pre(self):
        ...
        if self._has_control_operation():
            raise ValueError(
                "Control operations must be the first and only operation in a route."
            )
    ...
    def broadcast(self, event_type: Optional[EventType] = None) -> "RouteBuilder":
        ...
        """
        self._validate_pre()
        self.route_config.operations.append(
            {"_fn_type": "broadcast", "event": event_type}
        )
        return self
    ...
    def _add_on_event_handler(
        ...
        handler: Optional[OnEventHandler],
        ...
        if method_name == "suspend":
            ...
            raise ValueError(f"Unknown handler method: {method_name}")

        if event_type in handlers:
            raise ValueError(f"Event {event_type} already registered for {method_name}")

        wrapped_handler = _EventHandlerDict(handler) if handler is not None else None
        handlers[event_type] = wrapped_handler

        # Add implicit handler to the bridge to handle the suspend.
        if method_name == "suspend":
            self.bridge.on(event_type)._add_control_operation(
                lambda message: self.route_handler._suspend(message, wrapped_handler)
            )
        elif method_name == "resume":
            self.bridge.on(event_type)._add_control_operation(
                lambda message: self.route_handler._resume(message, wrapped_handler)
            )
        elif method_name == "interrupt":
            self.bridge.on(event_type)._add_control_operation(
                lambda message: self.route_handler._interrupt(message, wrapped_handler)
            )
        return self

    def suspend_on(
        self, event_type: EventType, handler: Optional[OnEventHandler] = None
    ) -> "RouteBuilder":
    ...
    def on(self, event: EventTypeOrAlias) -> "RouteBuilder":
...
class RouteHandler:
    ...
    def should_process_message(self, message: Message) -> bool:
        ...
        if (
            ...
            or len(self.route_config.operations) == 0
        ):
            return False
        ...
        for prop_name, expected_value in event_filters.items():
            if not hasattr(message.event, prop_name):
                logger.debug(
                    f"Event {message.event} does not have property {prop_name}"
                )
                return False
            ...
            async with self._task_lock:
                if self.route_config.max_concurrent_tasks is not None:
                    ...
                    if (
                        ...
                        >= self.route_config.max_concurrent_tasks
                    ):
                ...
                task = asyncio.create_task(
                    self._process_operations(message, self.route_config.operations)
                )
    ...
    async def _process_operations(self, data: Any, operations: List[dict]) -> Any:
        ...
        for i, operation in enumerate(operations):
            ...
            if fn_type == "map":
                if fn is None:
                    raise ValueError("Map operation requires a callable")
                ...
                if fn is None:
                    raise ValueError("Filter operation requires a callable")
                if asyncio.iscoroutinefunction(fn):
    ...
    async def _handle_stream(
        ...
        try:
            if generator_fn is None:
                ...
                if generator_fn is None:
                    raise ValueError(
                        "Stream operation requires a generator or callable"
                    )
