...
def get_broker() -> ListQueueBroker:
    ...
    not require ``REDIS_URL``. The Taskiq CLI target
    ``agentkit_serve.workers.broker:broker`` still resolves: ``broker`` is
...
def __getattr__(name: str) -> Any:
    # PEP 562: resolve ``broker`` lazily so the Taskiq CLI target can getattr
    # it without building the broker at import.
    if name == "broker":
        return get_broker()
