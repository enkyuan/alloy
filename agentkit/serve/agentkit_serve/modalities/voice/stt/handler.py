...
from agentkit.infra.realtime.redis import RedisKeys, get_redis_stream_client
from agentkit.infra.events.envelope import (
    build_event_envelope,
    parse_event_envelope,
    to_redis_stream_fields,
)
from agentkit.modalities.voice.event_models import UserTranscriptionReceived
...
def _parse_pubsub_event(data: Any) -> dict[str, Any] | None:
    if isinstance(data, str):
        raw: bytes | bytearray | dict[str, Any] = data.encode("utf-8")
    ...
    try:
        return parse_event_envelope(raw).model_dump()
    except Exception:
        return None
...
async def publish_transcription(
    ...
    await redis_conn.xadd(
        RedisKeys.STREAM_AGENT_INPUT, cast(dict[Any, Any], stream_fields)
    )


async def forward_agent_updates(
    websocket: WebSocket,
    redis_conn: Any,
    user_id: str,
    timeout: float = 15.0,
    ...
    try:
        while True:
            ...
            event = _parse_pubsub_event(data)
            if event is None:
                continue
