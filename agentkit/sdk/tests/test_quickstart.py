...
async def test_quickstart_event_inspection() -> None:
    ...
    await store.append(agentkit.UserMessage(session_id="s2", content="hi"))
    await runtime.run_turn("s2")

    events = await store.get_events("s2")
    # Every event must be an AgentKitEvent (Pydantic model)
    for e in events:
...
async def test_quickstart_send_convenience_method() -> None:
    ...
    await store.append(agentkit.UserMessage(session_id="s3", content="first"))
