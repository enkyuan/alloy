...
All notable changes to the `agentkit` Python SDK are documented here.
...
- **CI workflow** — `.github/workflows/sdk-tests.yml` added (runs pyrefly +
  pytest on every push/PR touching `agentkit/sdk/`).

### Changed

- **`agentkit/core/config.py`** — Added `from __future__ import annotations`,
  `TYPE_CHECKING` guard with `settings: Settings` annotation, and `@overload`
  stubs for `__getattr__` so type checkers resolve `from agentkit.core.config
  import settings` as `Settings` instead of `Any`.
- **`agentkit/README.md`** — Status updated from "pre-release" to "pre-beta"
  with explicit scope on what is and isn't production-hardened.

### Fixed

- `agentkit/infra/realtime/redis.py` — redis.asyncio import is already lazy
  (uses `_get_redis_module()`); added `TYPE_CHECKING` guard and updated module
  docstring to document the `agentkit[realtime]` install requirement.
...
- CLI scaffold (`agentkit` entry point).
