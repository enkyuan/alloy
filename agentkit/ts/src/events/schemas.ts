/**
 * Zod schemas for all AgentKit events, mirroring
 * `agentkit.infra.events.schemas`. Field names stay snake_case because they are
...
export const AgentKitEvent = z.discriminatedUnion("type", [
  SessionCreated,
  SessionClosed,
  UserMessage,
  UserAudioChunk,
...
/** A validated AgentKit event (output type, with defaults applied). */
export type AgentKitEvent = z.infer<typeof AgentKitEvent>;

/** The input type accepted by `AgentKitEvent.parse` (defaults optional). */
export type AgentKitEventInput = z.input<typeof AgentKitEvent>;
