 ...
 * (`agentkit.infra.events.bus.EventBus`). Events are fanned out per session to
...
class Subscription implements AsyncIterableIterator<AgentKitEvent> {
  private readonly buffered: AgentKitEvent[] = [];
  private pending: ((r: IteratorResult<AgentKitEvent>) => void) | null = null;
  private closed = false;

  constructor(private readonly onReturn: () => void) {}

  /** Deliver an event to the consumer (or buffer it until they ask). */
  push(event: AgentKitEvent): void {
  ...
  next(): Promise<IteratorResult<AgentKitEvent>> {
  ...
  return(): Promise<IteratorResult<AgentKitEvent>> {
    this.close();
    this.onReturn();
    return Promise.resolve({ value: undefined, done: true });
  }
  ...
  [Symbol.asyncIterator](): AsyncIterableIterator<AgentKitEvent> {
    return this;
  }
}
