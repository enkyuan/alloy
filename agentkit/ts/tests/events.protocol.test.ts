...
class RecordingBus implements EventBusProtocol {
  readonly published: AgentKitEvent[] = [];
  async publish(event: AgentKitEvent): Promise<void> {
    this.published.push(event);
  }
  subscribe(_sessionId: string): AsyncIterableIterator<AgentKitEvent> {
...
describe("EventBusProtocol", () => {
  ...
  it("EventBus partitions events per session via the protocol", async () => {
    ...
    const otherSession = AgentKitEvent.parse({
      type: EventType.SESSION_CREATED,
      session_id: "s2",
    });
    ...
    await sub.return?.({ value: undefined, done: true } as IteratorResult<AgentKitEvent>);
    bus.close();
  });
});
