...
def _require_executor(executor: ToolExecutor) -> ToolExecutor:
    """Validate the single-invocation executor contract at construction."""
    if not callable(executor):
        raise TypeError("tool executor must be callable")
    try:
        parameters = tuple(inspect.signature(executor).parameters.values())
    except (TypeError, ValueError) as error:
        raise TypeError("tool executor must expose an inspectable signature") from error
    positional = [
        parameter
    ...
    if len(positional) != 1 or any(
        ...
        raise TypeError("tool executor must accept exactly one ToolInvocation")
    return executor
    ...
    def __init__(
        ...
        *,
        ...
        if approval_handler is not None and not isinstance(
            ...
            raise TypeError("approval handler must implement request()")
        self.executor = _require_executor(executor)
        self.policy = policy
