...
Both the embedding backend and the embedding cache are pluggable. Without an
``Embedder`` the retriever falls back to returning all tools. With an embedder,
vectors are cached in memory unless an ``EmbeddingCache`` is supplied.
"""
