...
def test_public_onboarding_uses_source_setup_for_unavailable_registry_packages() -> (
    None
):
    docs_root = REPO_ROOT / "apps" / "docs"
    paths = (
    ...
    for path in paths:
        source = " ".join(path.read_text().split())
        assert "PyPI" in source
        assert "July 25, 2026" in source
        assert "available" in source or "unavailable" in source
...
def test_maintained_public_docs_reject_pre_beta_contract_guidance() -> None:
    ...
    assert "git clone https://github.com/enkyuan/alloy.git" in getting_started
