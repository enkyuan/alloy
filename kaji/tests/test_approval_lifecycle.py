...
def test_planner_requires_typed_approval_handler() -> None:
    async def executor(_invocation: ToolInvocation) -> dict[str, bool]:
        return {"ok": True}

    async def boolean_callback(
        _tool_name: str,
    ...
    with pytest.raises(TypeError, match="implement request"):
        ToolPlanner(
            executor,
            approval_handler=cast(Any, boolean_callback),
        )
