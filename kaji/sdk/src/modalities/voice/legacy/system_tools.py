...
.. deprecated::
   ...
   (``kaji.runtime.tools.registry``). The tools live here in the voice
   subtree, not under ``runtime/tools/``, because they are voice-specific.
"""
...
from kaji.modalities.voice.event_models import AgentResponse, EndCall
from kaji.types.tool import ToolDefinition
...
class EndCallArgs(BaseModel):
    ...
    goodbye_message: str = Field(
        description="The final message to say before ending the call"
    )
...
class EndCallTool(ToolDefinition):
    ...
    def to_gemini_tool(cls) -> Any:
        ...
        tool_cls = getattr(gemini_types, "Tool")
        function_decl_cls = getattr(gemini_types, "FunctionDeclaration")
                ...
                function_decl_cls(
                    ...
                    parameters={
                        ...
                        "properties": {
                            "goodbye_message": {
                                "type": "string",
                                "description": EndCallArgs.model_fields[
                                    "goodbye_message"
                                ].description,
                            }
                        },
...
class TransferToolCall:
    ...
    def __init__(
        self, target_phone_numbers: List[str], description: Optional[str] = None
    ):
    ...
    def description(self) -> str:
        return (
            self._description
            or "Initiates a transfer of the call to the destination phone number."
        )
