...
SDIST="$(find "$DIST_DIR" -maxdepth 1 -type f \( -name '*.tar.gz' -o -name '*.zip' \) -print | head -1)"
if [ -z "$WHEEL" ] || [ -z "$SDIST" ]; then
  echo "FAIL: expected one wheel and one sdist under $DIST_DIR" >&2
  exit 1
fi
...
WHEEL="$WHEEL" SDIST="$SDIST" SDK_ROOT="$SDK_ROOT" CONTRACTS_DIR="$CONTRACTS_DIR" \
  "${PYTHON_CMD[@]}" - <<'PY'
...
def forbidden_artifacts(paths: set[str]) -> list[str]:
    ...
    for path in sorted(paths):
        parts = path.split("/")
        if any(part in {"__pycache__", ".pytest_cache", ".ruff_cache", "logs"} for part in parts):
...
def metadata_license(data: bytes, label: str) -> bytes:
    ...
    starts = [index for index, line in enumerate(lines) if line.startswith(b"License: ")]
    if len(starts) != 1:
...
def validate_wheel_metadata(data: bytes, label: str) -> None:
    ...
    setuptools_pin = next(
        (item.split("==", 1)[1] for item in build_requires if item.startswith("setuptools==")),
        None,
    )
    expected = {
        "Wheel-Version": ["1.0"],
...
with zipfile.ZipFile(wheel) as zf:
    ...
    if len(dist_info_roots) != 1:
        fail(f"wheel must contain one dist-info directory, found {sorted(dist_info_roots)}")
    ...
    license_paths = sorted(path for path in names if path.endswith("/LICENSE") or path == "LICENSE")
...
with tarfile.open(sdist, "r:*") as tf:
    ...
    members = {name: member for name, member in all_members.items() if member.isfile()}
    names = set(members)
    ...
    try:
        source_lines = sdist_bytes("kaji.egg-info/SOURCES.txt").decode("utf-8").splitlines()
    except UnicodeDecodeError:
    ...
    for source_path in source_lines:
        if checked_archive_path(source_path) != source_path:
            fail(f"sdist egg-info SOURCES.txt contains unsafe path: {source_path!r}")
    ...
    packaged_contracts = {
        ...
        if path.startswith("src/contracts/") and Path(path).suffix in {".json", ".md"}
    }
    if packaged_contracts != set(canonical_contracts):
        missing = sorted(set(canonical_contracts) - packaged_contracts)
        extra = sorted(packaged_contracts - set(canonical_contracts))
