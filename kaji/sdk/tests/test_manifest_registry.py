...
def test_python_and_typescript_manifest_schemas_match() -> None:
    repo_root = Path(__file__).resolve().parents[3]
    python_schema = json.loads(
        (repo_root / "kaji/sdk/src/integrations/registry/schema.json").read_text()
    )
    ts_schema = json.loads((repo_root / "kaji/ts/registry/schema.json").read_text())

    assert python_schema == ts_schema
...
def test_load_manifest_parses_peer_deps(
    ...
    (root / "index.json").write_text(
        '{"version": "0.1.0", "integrations": {"ts-deps": "ts-deps/manifest.json"}}'
    )
    (root / "ts-deps").mkdir()
    (root / "ts-deps" / "manifest.json").write_text(
        '{"name": "ts-deps", "version": "0.1.0", "namespace": "ts_deps", '
        '"description": "x", "auth": {"kind": "none"}, '
        '"files": ["index.ts"], '
        '"tools": [{"name": "x", "description": "x"}], '
        '"peerDeps": {"better-sqlite3": "^9"}}'
    )
    monkeypatch.setattr(ai, "_registry_root", lambda: root)

    manifest = load_manifest("ts-deps")

    assert manifest.peer_deps == {"better-sqlite3": "^9"}
