...
`kaji-sdk` (TS at `kaji/ts`, npm-published, dual ESM/CJS) is a typed agent SDK:
...
> 2026-08-06). Files landed under `kaji/ts/examples/agentos/`: `agentos-integration.ts`,
> `exec-agent.ts`, `README.md`, `package.json`, `tsconfig.json`, `.gitignore` + a one-line
> `kaji/ts/tsconfig.json` exclude. Core package untouched: `examples/` absent from
...
kaji `AgentRuntime`, living under `kaji/ts/examples/agentos/` (NOT in `src/`, NOT an
...
- README + `kaji/ts/CHANGELOG.md`: document as experimental. (docs-contract only asserts
...
Evidence both cited: `kaji/ts/package.json:4` self-describes as **"infra-free core"**;
