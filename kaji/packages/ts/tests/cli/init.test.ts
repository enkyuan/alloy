...
describe("kaji init", () => {
    ...
    try {
      ...
      switch (testCase.name) {
        ...
        case "template-agent":
          ...
          expect(stdout.join("\n")).toContain("Next:");
          break;
        case "template-capability":
          expect(readFileSync(join(project, "capability.ts"), "utf8")).toContain("Kaji.execute");
          expect(readFileSync(join(project, "capability.ts"), "utf8")).toContain("capability(");
          expect(existsSync(join(project, "agent.ts"))).toBe(false);
          expect(stdout.join("\n")).toContain("Next:");
          break;
        default:
  ...
  it("typechecks the capability scaffold generated by --template capability", async () => {
    const out = mkdtempSync(join(tmpdir(), "kaji-init-capability-scaffold-"));
    const code = await sourceInit(
      [out, "--provider", "mock", "--yes", "--template", "capability"],
      {
        registryRoot: "",
        log: () => {},
      },
    ...
    expect(code).toBe(0);
    const capabilityPath = join(out, "capability.ts");
    expect(existsSync(capabilityPath)).toBe(true);
    // Create the workdir inside packageRoot so node_modules resolution finds zod
    const workdir = mkdtempSync(resolve(import.meta.dirname, "../../.docs-contract-cap-"));
    try {
      writeFileSync(resolve(workdir, "capability.ts"), readFileSync(capabilityPath, "utf8"));
      writeFileSync(
        resolve(workdir, "tsconfig.json"),
        JSON.stringify({
          extends: resolve(import.meta.dirname, "../../tsconfig.json"),
          compilerOptions: { noEmit: true },
          include: ["capability.ts"],
        }),
      );

      // Typecheck must succeed — guards against type errors in generated scaffolds
      // (e.g. accessing .message on a JsonValue union).
      execFileSync(
        "node",
        [
