...
const packageRoot = resolve(repoRoot, "kaji/packages/ts");
...
describe("cross-SDK release matrix docs", () => {
  ...
  it("keeps stable core, experimental, and not-ported surfaces explicit", () => {
    const combined = [
      read("kaji/RELEASE_MATRIX.md"),
      read("kaji/README.md"),
      read("kaji/packages/ts/README.md"),
      read("docs/MVP.md"),
    ].join("\n");
  ...
  it("matches both integration registry stability indexes", () => {
    const matrix = read("kaji/RELEASE_MATRIX.md");
    const python = JSON.parse(read("kaji/packages/py/src/integrations/registry/index.json")) as {
      integrations: Record<string, { stability: string; runtimes: string[] }>;
    };
    const typescript = JSON.parse(read("kaji/packages/ts/registry/index.json")) as {
      integrations: Record<string, { stability: string; runtimes: string[] }>;
    };
