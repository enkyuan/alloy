...
import { Integration, listToolSpecs, ToolRegistry } from "@irogane/kaji";
import type { FixedOriginRequester } from "@irogane/kaji/integrations";
...
describe("GitHub registry bundle", () => {
  ...
  it("keeps package ABI version domains distinct from npm and the shared manifest", () => {
    const packageManifest = JSON.parse(
      readFileSync(resolve(kajiRoot, "packages/ts/package.json"), "utf8"),
    );
    const copiedManifest = JSON.parse(
      readFileSync(resolve(kajiRoot, "packages/ts/registry/github/manifest.json"), "utf8"),
    );
