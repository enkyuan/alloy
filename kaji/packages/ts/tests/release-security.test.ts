...
const requiredGateCommand =
  "uv run --project kaji/packages/py --no-sync python kaji/scripts/beta_release_check.py --gate";
...
describe("Kaji workflow contracts", () => {
  ...
  it("sets up frozen Python dependencies before central evidence validation", () => {
    ...
    expect(validationSteps[0]?.run).toContain(
      "uv run --project kaji/packages/py --no-sync python kaji/scripts/validate_release_evidence.py",
    );
