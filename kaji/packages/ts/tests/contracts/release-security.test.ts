...
const requiredGateCommand =
  "uv run --project kaji/packages/py --no-sync python -m kaji.tooling.release.check --gate";
...
function assertProtectionReadyGate(workflow: Workflow): void {
  ...
  expect(jobId).toBe("kaji-pr-gate");
  expect(job.name).toBe("release gate");
  ...
  ).toMatchObject({ "working-directory": ".", "sync-args": "--frozen" });
  expect(
...
function gateJob(workflow: Workflow): WorkflowJob {
  const job = workflow.jobs?.["kaji-pr-gate"];
  if (!job) throw new Error("missing Kaji release gate job");
  return job;
}
...
function assertTrustedHandoffWorkflow(workflow: Workflow, source: string): void {
  ...
  const stageIndex = stageSteps.findIndex((step) =>
    step.run?.includes("trusted/kaji/tooling/release/typescript/handoff.py stage"),
  );
...
function signedBetaFixture() {
  const tagName = "kaji-v0.3.0-alpha.1";
  ...
  return {
    ...
    candidate: artifact(candidateArtifactId, "kaji-artifacts", candidateArtifactDigest),
    evidence: artifact(
      evidenceArtifactId,
...
async function runSignedTagParser(
  ...
  await run(
    ...
    },
    {
      ...
      eventName: "push",
      ref: "refs/tags/kaji-v0.3.0-alpha.1",
      payload: {
        repository: {
          private: false,
          default_branch: "main",
...
function runCompositeTagReverification(
  ...
  const fixture = signedBetaFixture();
  mutate?.(fixture);
  const action = readYaml(".github/actions/verify-kaji-tag/action.yml").value;
  ...
  try {
    const completed = spawnSync("/bin/bash", ["-c", step.run!], {
      ...
      env: {
        ...
        EXPECTED_CANDIDATE_ARTIFACT_NAME: "kaji-artifacts",
        ...
        EXPECTED_NPM_TARBALL_NAME: "irogane-kaji-0.3.0-alpha.1.tgz",
...
function onboardingBindingFixture(workflowName: OnboardingConsumerWorkflow) {
  ...
  const headBranch = publish ? "kaji-v0.3.0-alpha.1" : "main";
  const expected = {
    producer: {
      id: 702_001,
      name: "kaji-artifacts",
  ...
  try {
    const completed = spawnSync("/bin/bash", ["-c", step.run!], {
      ...
      env: {
        ...
        RELEASE_TAG: "kaji-v0.3.0-alpha.1",
        RUNNER_TEMP: root,
        WORKFLOW_SHA: commit,
      },
    });
...
function runExactVersionRegistryAbsence(
  ...
  const fixture: ExactVersionRegistryFixture = {
    ...
    targetEffectiveUrl: "https://registry.npmjs.org/@irogane%2Fkaji/0.3.0-alpha.1",
    targetRedirects: "0",
...
case "$url" in
  ...
  https://registry.npmjs.org/@irogane%2Fkaji)
    ...
    ;;
  https://registry.npmjs.org/@irogane%2Fkaji/0.3.0-alpha.1)
    key=TARGET
...
describe("Kaji workflow contracts", () => {
  ...
  it.each([
    ["python.test.yml", ["kaji/**", "kaji/packages/serve/**"]],
    ["ts.test.yml", ["kaji/packages/ts/**", "ryo/auth/**", "packages/ui/**"]],
  ...
  it("pins and annotates every external action used by Kaji CI", () => {
    ...
    expect(closure).toContain(".github/actions/verify-kaji-tag/action.yml");
  });
  ...
  it("requires signature-bound authorization at every npm release mutation boundary", () => {
    ...
    const action = readYaml(".github/actions/verify-kaji-tag/action.yml");
  ...
  it.each([
    ...
    (_label, mutate) => {
      ...
      expect(completed.endpoints).toEqual([
        `repos/enkyuan/alloy/git/ref/tags/kaji-v0.3.0-alpha.1`,
  ...
  it("authenticates raw signed rehearsal evidence before extraction and carrier fanout", () => {
    ...
    for (const fragment of [
      "kaji.tooling.release.validate.evidence",
      "--mode rehearsal",
  ...
  it("binds the current TypeScript candidate to alpha.1 and preserves prior incident history", () => {
    ...
    expect(packageManifest.version).toBe("0.3.0-alpha.1");
    ...
    expect(tarball).toBe("irogane-kaji-0.3.0-alpha.1.tgz");
    if (existsSync(resolve("dist"))) {
      ...
      for (const relativePath of exportedIdentityPaths) {
        ...
        expect(output, relativePath).toMatch(/(?:var|declare const) VERSION = "0\.3\.0-alpha\.1"/);
    ...
    expect(changelog).toContain("## [0.3.0-alpha.1] - 2026-08-01");
  ...
  it("smokes compatibility matrices only from verified producer artifacts", () => {
    ...
    for (const [workflow, producer] of [
      ...
      for (const [jobId, smokeScript] of [
        ["python-compat", "kaji.tooling.release.smoke"],
        ["node-compat", "kaji/packages/ts/scripts/smoke_package.mts"],
      ] as const) {
    ...
    for (const [workflowName, producer] of [
      ...
      const assertProtectedNodeJob = (job: WorkflowJob) => {
        ...
        expect(steps[finalizer]?.env).toMatchObject({
          KAJI_COMPAT_CANDIDATE_TARBALL: ".artifacts/kaji-release/irogane-kaji-0.3.0-alpha.1.tgz",
  ...
  it("keeps supplied-artifact handoff children token-free and nonpacking", () => {
    const smoke = readFileSync(
      resolve(repositoryRoot, "kaji/packages/ts/scripts/smoke_package.mts"),
      "utf8",
    );
    const installedProof = readFileSync(
      resolve(repositoryRoot, "kaji/packages/ts/scripts/installed-github-smoke.mts"),
      "utf8",
    );
  ...
  it("authenticates the all-or-none publisher receipt output tuple before exact-ID download", () => {
    ...
    for (const fragment of [
      ...
      ".run_attempt == 1",
      '.path == ".github/workflows/kaji.publish.yml"',
      '.head_branch == "kaji-v0.3.0-alpha.1"',
      ".head_sha == $commit",
      ".id == $id",
      ".name == $name",
      ".digest == $digest",
  ...
  it("uses exact-version HTTPS registry absence responses instead of E404 text matching", () => {
    const { source, workflow } = readWorkflow("kaji.publish.yml");
    expect(source).not.toContain("npm view kaji@0.3.0-alpha.1");
    ...
    expect(classifier.run).toContain("https://registry.npmjs.org/@irogane%2Fkaji/0.3.0-alpha.1");
    ...
    for (const step of [preflight, immediate, classifier]) {
      expect(step.run).toContain("https://registry.npmjs.org/@irogane%2Fkaji/0.3.0-alpha.1");
    }
    expect(classifier.run).toContain('.name == "@irogane/kaji"');
    expect(classifier.run).toContain('.version == "0.3.0-alpha.1"');
    ...
    for (const [jobId, stepName] of [
      ...
      for (const [label, overrides] of [
        ...
        [
          ...
          // distinct from the requested target (.../0.3.0-alpha.1) so the effective_url
  ...
  it("publishes and byte-verifies npm only while requiring PyPI absence", () => {
    ...
    expect(registryVerifier.run).toContain("kaji.tooling.release.verify.published");
    ...
    expect(classifier.run).toContain("kaji.tooling.release.verify.published state");
    expect(classifier.run).toContain("--target npm");
  ...
  it("uses Bun's supported cwd syntax when rebuilding clean-checkout packages", () => {
    ...
    for (const [jobId, stepName] of [
      ...
      const job = workflow.jobs?.[jobId];
      expect(job, jobId).toBeDefined();
      expect(workflowStep(job!, stepName).run).toContain("bun run --cwd kaji/packages/ts build");
    }
    expect(source).not.toContain("bun --cwd kaji/packages/ts run build");
  });
  ...
  it("runs protected benchmark and provider consumers only from installed candidate bytes", () => {
    ...
    const paired = performance.jobs?.["paired-replica"]?.steps?.find((step) =>
      step.run?.includes("kaji.tooling.performance.bench.pairs"),
    )?.run;
    ...
    const soak = performance.jobs?.soak?.steps?.find((step) =>
      step.run?.includes("kaji.tooling.performance.run.soak"),
    )?.run;
    ...
    for (const workflowName of ["kaji.rehearsal.yml", "kaji.publish.yml"] as const) {
      ...
      const command = provider?.steps?.find((step) =>
        step.run?.includes("kaji.tooling.providers.openai.live"),
      )?.run;
  ...
  it("centrally validates every current-run rehearsal receipt", () => {
    ...
    const validation = steps.find((step) =>
      step.run?.includes("kaji.tooling.release.validate.evidence"),
    );
  ...
  it("centrally validates and attests every current-run publish receipt", () => {
    ...
    expect(attachedPaths).toEqual([
      ".artifacts/kaji-release/irogane-kaji-0.3.0-alpha.1.tgz",
      ...
      ".artifacts/kaji-publication-status/downloaded/registry-irogane-kaji-0.3.0-alpha.1.tgz",
      ".artifacts/kaji-publication-status/downloaded/registry-irogane-kaji-0.3.0-alpha.1.tgz.github-attestation.json",
  ...
  it("sets up frozen Python dependencies before central evidence validation", () => {
    ...
    expect(validationSteps[0]?.run).toContain(
      "uv run --project kaji/packages/py --no-sync python -m kaji.tooling.release.validate.evidence",
    );
  ...
  it("keeps paired performance on measured GitHub-hosted macOS images", () => {
    ...
    const runtimeGuard = readFileSync(
      resolve(repositoryRoot, "kaji/tooling/performance/platform.py"),
      "utf8",
    );
    for (const guard of [
