...
def test_github_manifest_is_beta_and_declares_native_owner_bundles() -> None:
    python = json.loads(
        (
            ROOT / "kaji/packages/py/src/integrations/registry/github/manifest.json"
        ).read_text()
    )
    typescript = json.loads(
        (ROOT / "kaji/packages/ts/registry/github/manifest.json").read_text()
    )
    python_index = json.loads(
        (ROOT / "kaji/packages/py/src/integrations/registry/index.json").read_text()
    )
    typescript_index = json.loads(
        (ROOT / "kaji/packages/ts/registry/index.json").read_text()
    )
...
def test_github_owner_fixture_has_one_closed_cross_runtime_shape() -> None:
    python_path = (
        ROOT / "kaji/packages/py/src/integrations/registry/github/owner-fixtures.json"
    )
    typescript_path = ROOT / "kaji/packages/ts/registry/github/owner-fixtures.json"
