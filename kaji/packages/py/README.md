...
This is the Python embedded agent SDK. It is distributed as `kaji` and
imported as `kaji`, with its source under `src`. It participates in a
`uv` workspace rooted at the repository root alongside the `kaji-serve`
reference service.

Canonical contracts, release scripts, benchmarks, and fixtures live once at
the Kaji root and are consumed by both SDKs. The Python SDK never imports
service-only code.
...
release, but Python package version `0.2.0b1` is not published to PyPI and
must not be presented as a registry install yet.

For Python SDK development from this repository, run `uv sync` from the
repository root and select this workspace member:

```bash
uv sync --package kaji --extra openai
```

Optional source-checkout extras remain available for experimental development:

```bash
uv sync --package kaji --extra anthropic  # Anthropic (experimental/WIP)
uv sync --package kaji --extra gemini     # Gemini provider
uv sync --package kaji --extra realtime   # Redis event bus (multi-process)
uv sync --package kaji --extra providers  # all provider SDKs
```

The future PyPI distribution name is `kaji`; installed code is imported as
...
Run these from the repository root; `uv sync` resolves the whole workspace and
`--package kaji` selects this member:

```bash
uv sync                                              # creates .venv, installs deps + dev group
uv run --package kaji pytest tests/              # no API keys required
uv run --package kaji python scripts/check_types.py  # static type check for the src/ remap
uv run --package kaji ruff check src             # lint
```
...
```bash
uv sync --package kaji --extra openai
OPENAI_API_KEY=... uv run --package kaji pytest -m integration tests/integration/test_openai_provider.py
OPENAI_API_KEY=... KAJI_LIVE_OPENAI_MODEL=gpt-5.4-mini \
  uv run --package kaji pytest -m integration tests/integration/test_openai_tools.py

uv sync --package kaji --extra anthropic
ANTHROPIC_API_KEY=... uv run --package kaji pytest -m integration tests/integration/test_anthropic_provider.py
```

The SDK test suite needs no environment. The service tests under
`kaji/packages/serve/tests/` cover the FastAPI REST and STT edge; those need Postgres
(see [`kaji-serve` README](https://github.com/enkyuan/alloy/blob/main/kaji/packages/serve/README.md)).
...
```bash
uv run --package kaji pytest -m "not integration"
```
...
```bash
uv sync --package kaji --extra realtime
...
| Experimental cross-process event delivery | `kaji[realtime]` + `EventBus` |
...
[`kaji-serve`](https://github.com/enkyuan/alloy/blob/main/kaji/packages/serve/README.md) package.
...
kaji/
├── pyproject.toml
├── src/              # Python distribution source
...
The repo contains **two Python distributions**: `kaji` (this SDK, imported
as `kaji`) and
[`kaji-serve`](https://github.com/enkyuan/alloy/blob/main/kaji/packages/serve/README.md) (the reference FastAPI + STT
