...
This is the Python embedded agent SDK. It is distributed as `kaji-sdk` and
imported as `kaji`, with its source under `src/kaji`. It participates in a
...
```bash
uv sync --package kaji-sdk --extra openai
```

Optional source-checkout extras remain available for experimental development:

```bash
uv sync --package kaji-sdk --extra anthropic  # Anthropic (experimental/WIP)
uv sync --package kaji-sdk --extra gemini     # Gemini provider
uv sync --package kaji-sdk --extra realtime   # Redis event bus (multi-process)
uv sync --package kaji-sdk --extra providers  # all provider SDKs
```
...
`--package kaji-sdk` selects this member:

```bash
uv sync                                              # creates .venv, installs deps + dev group
uv run --package kaji-sdk pytest tests/              # no API keys required
uv run --package kaji-sdk python scripts/check_types.py  # static type check for the src/ remap
uv run --package kaji-sdk ruff check src             # lint
```
...
```bash
uv sync --package kaji-sdk --extra openai
OPENAI_API_KEY=... uv run --package kaji-sdk pytest -m integration tests/integration/test_openai_provider.py
OPENAI_API_KEY=... KAJI_LIVE_OPENAI_MODEL=gpt-5.4-mini \
  uv run --package kaji-sdk pytest -m integration tests/integration/test_openai_tools.py

uv sync --package kaji-sdk --extra anthropic
ANTHROPIC_API_KEY=... uv run --package kaji-sdk pytest -m integration tests/integration/test_anthropic_provider.py
```
...
```bash
uv run --package kaji-sdk pytest -m "not integration"
```
...
```bash
uv sync --package kaji-sdk --extra realtime
