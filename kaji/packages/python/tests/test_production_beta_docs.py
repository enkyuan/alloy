...
def test_exact_python_onboarding_echo_snippet_runs_offline(tmp_path: Path) -> None:
    ...
    echo.mkdir()
    shutil.copyfile(
        REPO_ROOT / "kaji/packages/python/src/kaji/integrations/registry/echo/echo.py",
        echo / "echo.py",
    )
...
def test_astro_docs_keep_status_motion_and_icon_contracts_explicit() -> None:
    ...
    assert "<code>npm i kaji-sdk@0.2.0-beta.11 zod</code>" in install_snippet
    assert 'type="button"' in install_snippet
    assert 'aria-label="Copy npm install command"' in install_snippet
    assert 'data-copy="npm i kaji-sdk@0.2.0-beta.11 zod"' in install_snippet
...
def test_public_onboarding_uses_published_npm_beta_and_defers_pypi() -> None:
    ...
    assert "https://www.npmjs.com/package/kaji-sdk/v/0.2.0-beta.11" in combined
    assert "npm install kaji-sdk@0.2.0-beta.11 zod openai" in combined
    assert "bun add kaji-sdk@0.2.0-beta.11 zod openai" in combined
...
def test_npm_install_copy_stays_after_the_final_hero_period() -> None:
    ...
    assert "npm i kaji-sdk@0.2.0-beta.11 zod" in overview
...
def test_task16_docs_cover_the_operating_contract_without_promotion_claims() -> None:
    ...
    repository_status_docs = [
        ...
        REPO_ROOT / "kaji" / "packages" / "python" / "CHANGELOG.md",
        REPO_ROOT / "kaji" / "packages" / "typescript" / "CHANGELOG.md",
...
def test_package_readmes_have_permanent_status_neutral_canonical_links() -> None:
    ...
    for path in (
        ...
        REPO_ROOT / "kaji" / "packages" / "typescript" / "README.md",
    ):
        text = path.read_text()
        marker = re.search(
...
def test_maintained_public_docs_reject_pre_beta_contract_guidance() -> None:
    ...
    assert "npm install kaji-sdk@0.2.0-beta.11 zod" in getting_started
...
def test_release_smokes_execute_the_marked_quickstart_blocks() -> None:
    ...
    ts_smoke = (
        REPO_ROOT / "kaji" / "packages" / "typescript" / "scripts" / "smoke_package.mts"
    ).read_text()
...
def test_typescript_onboarding_docs_make_only_the_protected_runner_claims() -> None:
    ...
    python_readme = (
        REPO_ROOT / "kaji" / "packages" / "python" / "README.md"
    ).read_text()
    typescript_readme = (
        REPO_ROOT / "kaji" / "packages" / "typescript" / "README.md"
    ).read_text()
...
def test_typescript_readme_quick_start_uses_an_esm_filename() -> None:
    readme = (REPO_ROOT / "kaji" / "packages" / "typescript" / "README.md").read_text()
...
def test_release_docs_enforce_the_npm_only_registry_boundary() -> None:
    ...
    assert (
        "uv run --project kaji/packages/python kaji init --provider openai"
    ...
    assert "npm install kaji-sdk@0.2.0-beta.11 zod openai" in public_docs
    assert "bun add kaji-sdk@0.2.0-beta.11 zod openai" in public_docs
    assert re.search(r"pip install [^\n`]*kaji-sdk", public_docs) is None
    typescript_readme = documents[
        REPO_ROOT / "kaji" / "packages" / "typescript" / "README.md"
    ]
    assert "npm install kaji-sdk@0.2.0-beta.11" in typescript_readme
