...
def test_packaged_schemas_match_canonical_contracts() -> None:
    for packaged in (
        REPO_ROOT / "kaji/packages/python/src/kaji/integrations/registry/schema.json",
        REPO_ROOT / "kaji/packages/typescript/registry/schema.json",
    ):
        assert (
    ...
    for packaged in (
        REPO_ROOT
        / "kaji/packages/python/src/kaji/integrations/registry/index.schema.json",
        REPO_ROOT / "kaji/packages/typescript/registry/index.schema.json",
    ):
...
def test_copy_provenance_is_closed_and_supports_demotion_detection() -> None:
    ...
    assert not validator.is_valid({**provenance, "abiSha256": None})
