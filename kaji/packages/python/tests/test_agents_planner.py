...
def test_tool_planner_requires_single_invocation_executor() -> None:
    async def invalid_executor(_name: str, _arguments: dict) -> dict:
        return {}

    with pytest.raises(TypeError, match="exactly one ToolInvocation"):
        ToolPlanner(invalid_executor)  # ty: ignore[invalid-argument-type]
...
async def test_tool_planner_emits_lifecycle_on_success():
    ...
    async def executor(invocation: ToolInvocation):
        return {
            "ok": True,
...
async def test_tool_planner_emits_failure_event():
    ...
    async def executor(_invocation: ToolInvocation):
...
async def test_tool_planner_generates_call_id_when_missing():
    ...
    async def _ok_executor(_invocation: ToolInvocation) -> str:
        return "ok"
