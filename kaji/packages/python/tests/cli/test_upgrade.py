...
def test_find_outdated_returns_only_upgradable() -> None:
    installed = {"kaji-sdk": "0.1.0", "kaji-serve": "0.1.0"}

    def fake(name: str) -> str | None:
        return {"kaji-sdk": "0.2.0", "kaji-serve": "0.1.0"}.get(name)

    out = find_outdated(installed, fake)
    assert out == [{"name": "kaji-sdk", "current": "0.1.0", "latest": "0.2.0"}]
...
def test_list_installed_kaji_excludes_the_unrelated_kaji_project(
    ...
    installed = [
        ...
        SimpleNamespace(metadata={"Name": "kaji-sdk"}, version="0.2.0b1"),
    ]
    monkeypatch.setattr(upgrade, "distributions", lambda: installed)

    assert upgrade.list_installed_kaji() == {"kaji-sdk": "0.2.0b1"}
