...
`kaji-sdk` (TS at `kaji/packages/typescript`, npm-published, dual ESM/CJS) is a typed agent SDK:
...
> 2026-08-06). Files landed under `kaji/packages/typescript/examples/agentos/`: `agentos-integration.ts`,
> `exec-agent.ts`, `README.md`, `package.json`, `tsconfig.json`, `.gitignore` + a one-line
> `kaji/packages/typescript/tsconfig.json` exclude. Core package untouched: `examples/` absent from
...
kaji `AgentRuntime`, living under `kaji/packages/typescript/examples/agentos/` (NOT in `src/`, NOT an
...
- `examples/agentos/package.json` — its OWN package.json, `"type":"module"`, pinning
  ...
  target) and `kaji-sdk` via `file:../..` or the published version.
...
`kaji-sdk` as a peer. This isolates all preview/native/ESM churn from kaji's core.
...
- **NOT** a `kaji-sdk/agentos` core subpath export. Breaks the infra-free/dual-format/
...
**Files (new, all in `kaji/packages/typescript/`):**
...
- README + `kaji/packages/typescript/CHANGELOG.md`: document as experimental. (docs-contract only asserts
...
**Files (new):** `src/agentos/bindings.ts` (or a `kaji-sdk/agentos` sub-export)
...
codebase and reached the same verdict: **do NOT ship `kaji-sdk/agentos` as a core subpath.**

Evidence both cited: `kaji/packages/typescript/package.json:4` self-describes as **"infra-free core"**;
...
|---|-------|----------|----------------|-----------|-----------|----------|
| 1 | CEO | Do not ship as core `kaji-sdk` subpath; prefer interop doc/example, then separate `@kaji/agentos` | USER CHALLENGE | P4 DRY / P6 action | Both models 6/6: preview+native+ESM-only dep corrupts an infra-free dual-format published contract; no demand signal | Core subpath (Options 1/3 as planned) |
...
| 9 | DX | Do not name a native/preview surface as a sibling of infra-free exports | USER CHALLENGE | P5 explicit | "experimental" tier can't encode non-portable; corrupts the honest package contract | `kaji-sdk/agentos` core subpath |
...
## Plan Summary
Three stacked ways to wire agentOS (a preview VM-isolation runtime) into kaji-sdk: (1) a
`kaji-sdk/agentos` code-exec Integration subpath, (2) hosting kaji's runtime as an agentOS
...
**You said:** build all three options *on top of agentOS, integrated into kaji-sdk*, and
...
contract, confused Windows/Alpine/CJS bug reports against kaji-sdk, a preview dep forcing
