...
const requiredGateCommand =
  "uv run --project kaji/packages/python --no-sync python kaji/scripts/beta_release_check.py --gate";
  ...
  try {
    const completed = spawnSync("/bin/bash", ["-c", step.run!], {
      ...
      env: {
        ...
        EXPECTED_NPM_TARBALL_NAME: "kaji-sdk-0.2.0-beta.11.tgz",
...
case "$url" in
  ...
  https://registry.npmjs.org/kaji-sdk)
    ...
    ;;
  https://registry.npmjs.org/kaji-sdk/0.2.0-beta.11)
    key=TARGET
...
describe("Kaji workflow contracts", () => {
  ...
  it.each([
    ...
    ["ts.test.yml", ["kaji/packages/typescript/**", "ryo/auth/**", "packages/ui/**"]],
  ...
  it("binds the current TypeScript candidate to beta.11 and preserves prior incident history", () => {
    ...
    expect(tarball).toBe("kaji-sdk-0.2.0-beta.11.tgz");
  ...
  it("smokes compatibility matrices only from verified producer artifacts", () => {
    ...
    for (const [workflow, producer] of [
      ...
      for (const [jobId, smokeScript] of [
        ...
        ["node-compat", "kaji/packages/typescript/scripts/smoke_package.mts"],
      ] as const) {
    ...
    for (const [workflowName, producer] of [
      ...
      const assertProtectedNodeJob = (job: WorkflowJob) => {
        ...
        const smoke = steps.findIndex((step) =>
          step.run?.includes("kaji/packages/typescript/scripts/smoke_package.mts"),
        );
        ...
        expect(steps[finalizer]?.env).toMatchObject({
          KAJI_COMPAT_CANDIDATE_TARBALL: ".artifacts/kaji-release/kaji-sdk-0.2.0-beta.11.tgz",
  ...
  it("keeps supplied-artifact handoff children token-free and nonpacking", () => {
    const smoke = readFileSync(
      resolve(repositoryRoot, "kaji/packages/typescript/scripts/smoke_package.mts"),
      "utf8",
    );
    const installedProof = readFileSync(
      resolve(repositoryRoot, "kaji/packages/typescript/scripts/installed-github-smoke.mts"),
      "utf8",
    );
  ...
  it("uses exact-version HTTPS registry absence responses instead of E404 text matching", () => {
    const { source, workflow } = readWorkflow("kaji.publish.yml");
    expect(source).not.toContain("npm view kaji-sdk@0.2.0-beta.11");
    ...
    expect(classifier.run).toContain("https://registry.npmjs.org/kaji-sdk/0.2.0-beta.11");
    ...
    for (const step of [preflight, immediate, classifier]) {
      expect(step.run).toContain("https://registry.npmjs.org/kaji-sdk/0.2.0-beta.11");
    }
    ...
    for (const [jobId, stepName] of [
      ...
      for (const [label, overrides] of [
        ...
        [
          ...
          { targetEffectiveUrl: "https://registry.npmjs.org/kaji-sdk/0.2.0-beta.0" },
        ],
  ...
  it("publishes and byte-verifies npm only while requiring PyPI absence", () => {
    ...
    expect(releaseAttach).toContain("kaji-sdk-0.2.0-beta.11.tgz");
    for (const forbidden of [
      "kaji_sdk-0.2.0b1-py3-none-any.whl",
      "kaji_sdk-0.2.0b1.tar.gz",
      "registry-kaji_sdk",
  ...
  it("uses Bun's supported cwd syntax when rebuilding clean-checkout packages", () => {
    ...
    for (const [jobId, stepName] of [
      ...
      const job = workflow.jobs?.[jobId];
      expect(job, jobId).toBeDefined();
      expect(workflowStep(job!, stepName).run).toContain(
        "bun run --cwd kaji/packages/typescript build",
      );
    }
    expect(source).not.toContain("bun --cwd kaji/packages/typescript run build");
  });
  ...
  it("centrally validates and attests every current-run publish receipt", () => {
    ...
    expect(attachedPaths).toEqual([
      ".artifacts/kaji-release/kaji-sdk-0.2.0-beta.11.tgz",
      ...
      ".artifacts/kaji-publication-status/downloaded/registry-kaji-sdk-0.2.0-beta.11.tgz",
      ".artifacts/kaji-publication-status/downloaded/registry-kaji-sdk-0.2.0-beta.11.tgz.github-attestation.json",
  ...
  it("sets up frozen Python dependencies before central evidence validation", () => {
    ...
    expect(validationSteps[0]?.run).toContain(
      "uv run --project kaji/packages/python --no-sync python kaji/scripts/validate_release_evidence.py",
    );
