...
const packageRoot = resolve(repoRoot, "kaji/packages/typescript");
...
describe("cross-SDK release matrix docs", () => {
  ...
  it("defines privileged journal recovery and disposal boundaries", () => {
    const readme = read("kaji/packages/typescript/README.md");
  ...
  it("keeps stable core, experimental, and not-ported surfaces explicit", () => {
    const combined = [
      read("kaji/RELEASE_MATRIX.md"),
      read("kaji/README.md"),
      read("kaji/packages/typescript/README.md"),
      read("docs/MVP.md"),
    ].join("\n");
    ...
    const tsReadme = read("kaji/packages/typescript/README.md");
  ...
  it("documents, typechecks, and failure-tests read-only packaged GitHub wiring", async () => {
    ...
    const registry = JSON.parse(read("kaji/packages/typescript/registry/index.json")) as {
  ...
  it("matches both integration registry stability indexes", () => {
    ...
    const python = JSON.parse(
      read("kaji/packages/python/src/kaji/integrations/registry/index.json"),
    ) as {
      integrations: Record<string, { stability: string; runtimes: string[] }>;
    };
    const typescript = JSON.parse(read("kaji/packages/typescript/registry/index.json")) as {
      integrations: Record<string, { stability: string; runtimes: string[] }>;
    };
