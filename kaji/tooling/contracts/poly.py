...
PYTHON_EXPORTER = PYTHON_SDK / "tooling" / "contracts" / "export.py"
...
def main() -> int:
    with tempfile.TemporaryDirectory(prefix="kaji-parity-") as directory:
        ...
        try:
            ...
            scenarios_doc = load_json(SCENARIOS.read_bytes(), str(SCENARIOS))
            python_only = {
                scenario["id"]
                for scenario in scenarios_doc["scenarios"]
                if scenario.get("runtime") == "python"
            }
            ...
            python_expected = {scenario["id"]: scenario for scenario in expected["scenarios"]}
            typescript_expected = {
                scenario["id"]: scenario
                for scenario in expected["scenarios"]
                if scenario["id"] not in python_only
            }
            python_actual = {scenario["id"]: scenario for scenario in python_normalized["scenarios"]}
            typescript_actual = {scenario["id"]: scenario for scenario in typescript_normalized["scenarios"]}
            if set(python_expected) != set(python_actual):
                missing = sorted(set(python_expected) - set(python_actual))
                extra = sorted(set(python_actual) - set(python_expected))
                raise ParityError(
                    f"Python exporter scenario divergence; missing={missing}, extra={extra}"
                )
            if set(typescript_expected) != set(typescript_actual):
                missing = sorted(set(typescript_expected) - set(typescript_actual))
                extra = sorted(set(typescript_actual) - set(typescript_expected))
                raise ParityError(
                    f"TypeScript exporter scenario divergence; missing={missing}, extra={extra}"
                )
            for scenario_id in typescript_expected:
                compare(
                    f"Python -> expected [{scenario_id}]",
                    python_expected[scenario_id]["snapshot"],
                    python_actual[scenario_id]["snapshot"],
                )
                compare(
                    f"TypeScript -> expected [{scenario_id}]",
                    typescript_expected[scenario_id]["snapshot"],
                    typescript_actual[scenario_id]["snapshot"],
                )
                compare(
                    f"Python -> TypeScript [{scenario_id}]",
                    python_actual[scenario_id]["snapshot"],
                    typescript_actual[scenario_id]["snapshot"],
                )
            for scenario_id in set(python_expected) - set(typescript_expected):
                compare(
                    f"Python -> expected [{scenario_id}]",
                    python_expected[scenario_id]["snapshot"],
                    python_actual[scenario_id]["snapshot"],
    ...
    print(
        ...
        f"{len(typescript_expected)} shared and {len(set(python_expected) - set(typescript_expected))} Python-only parity scenarios"
    )
    return 0
