...
from kaji.tooling.performance.platform import (
...
GATE = (next(parent for parent in Path(__file__).resolve().parents if (parent / "contracts").is_dir() and (parent / "packages").is_dir())) / "tooling/performance/gate/benchmark.py"
...
def main() -> int:
    ...
    if args.mode == "quick":
        status = run(
            [
                *pytest,
                str(SDK / "tests" / "runtime" / "test_runtime_complexity.py"),
                str(SDK / "tests" / "runtime" / "test_runtime_faults.py"),
                str(SDK / "tests" / "events" / "test_events_journal.py"),
                str(SDK / "tests" / "runtime" / "test_runtime_concurrency.py"),
                str(SDK / "tests" / "tools" / "test_tool_execution_limits.py"),
                str(SDK / "tests" / "agents" / "test_approval_lifecycle.py"),
                "-q",
                "--no-cov",
            ]
        )
        ...
        status = run(
            [
                ...
                "tests/events/delivery.test.ts",
                "tests/runtime/concurrency.test.ts",
                "tests/tools/execution-limits.test.ts",
                "tests/agents/lifecycle.test.ts",
                "tests/integrations/safe-fetch.test.ts",
            ],
            cwd=TYPESCRIPT,
        )
        if status != 0:
