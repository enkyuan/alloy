...
```console
uv run --project kaji/packages/py python kaji/scripts/check_event_migration.py path/to/events.jsonl
```
...
`kaji` supports Zod `>=4.3 <5` as a peer dependency. Upgrade Zod before the
...
function requireZod4(version: string): void {
  ...
  if (major !== 4) throw new Error("kaji requires Zod 4");
}
