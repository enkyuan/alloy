...
- `kaji/packages/py/src/runtime/agents/runtime.py` -- `tools: List[ToolSpec]` constructor arg, provider-neutral payload surfaced to `generate_stream` each turn.
- Tests: `kaji/tests/test_agents_runtime.py`.

### 2. Register the mock provider (DONE)

- `kaji/packages/py/src/runtime/providers/registry.py` -- loads mock in `_ensure_builtin_providers_loaded`.
- `kaji/packages/py/src/runtime/providers/mock.py` -- self-registers, drives full tool loop with no network.

### 3. Export the agent loop from the public API (DONE)

- `kaji/packages/py/src/__init__.py` -- `AgentRuntime`, `AgentStrategy`, `ToolPlanner`, `CancellationToken`, `ModelProvider`, `UserMessage`, `InMemoryEventBus` added to lazy map.

### 3b. In-memory `EventBus` (DONE)

- `kaji/packages/py/src/infra/events/bus.py` -- `InMemoryEventBus` (per-session log + live fan-out). Redis `EventBus` unchanged.
...
- `kaji/packages/py/src/runtime/providers/openai.py` -- `generate` + `generate_stream` with tool calls via async openai SDK. Kimi stays default.
- Tests: `kaji/tests/test_providers_openai.py`.

### 5. Anthropic provider (DONE)

- `kaji/packages/py/src/runtime/providers/anthropic.py` -- Messages API provider with streaming + tool use.
...
- `kaji/packages/py/src/runtime/providers/gemini.py` -- `generate_stream` now has full history + tool calls parity with `generate`.
- Tests: `kaji/tests/test_providers_gemini_stream.py`.

### 7. Provider-neutral tool payloads (DONE)

- `kaji/packages/py/src/runtime/tools/payload.py` -- flat neutral list, `to_gemini` / `to_openai` translators at provider boundaries.
- Tests: `kaji/tests/test_tools_payload.py`.

### 7b. Fix Kimi tool translation (DONE)

- `kaji/packages/py/src/runtime/providers/kimi.py` -- translates via `to_openai` at the boundary.
...
- `kaji/packages/py/src/runtime/tools/retriever.py` -- pluggable `Embedder` + `EmbeddingCache` protocols. Defaults infra-free.
- `kaji/packages/py/src/infra/realtime/embedding_cache.py` -- `RedisEmbeddingCache` opt-in.
- Tests: `kaji/tests/test_tools_retriever.py`.

### 11. Conversation history storage primitives (DONE)

- `kaji/packages/py/src/runtime/agents/history.py` -- `HistoryStore` protocol + `InMemoryHistoryStore`.
- `kaji/packages/py/src/infra/realtime/redis_history.py` -- `RedisHistoryStore` opt-in.
- Tests: `kaji/tests/test_agents_history.py`, `test_redis_history.py`.

### 11b. Project `TOOL_CALL_FAILED` into session state (DONE)

- `kaji/packages/py/src/infra/events/replay.py` -- `TOOL_CALL_FAILED` branch appends `{role: tool, name, content: "Error: <error>"}`.
...
- Implementation in `kaji/packages/py/src/tools/payment.py` (or equivalent) -- thin HTTP call to ryo API, configurable base URL via env.
...
- `kaji/packages/py/src/knowledge/` -- `Document`/`Chunk` types, deterministic `chunk_text`, `VectorStore` protocol + `InMemoryVectorStore` (cosine, dimension-guarded), `DocumentRAG` (ingest + retrieve). Infra-free by default; reuses the tool retriever's `Embedder` protocol.
...
- `kaji/packages/py/src/runtime/sessions/store.py` -- `SessionStore` protocol + `SessionRecord` + `InMemorySessionStore` (a cross-session index, distinct from the per-session `EventStore`).
...
- Exported from the public API; runnable quickstart in the SDK README.
- `kaji/packages/serve/src/server/session_store.py` -- `PostgresSessionStore` implements the same protocol over the existing `Conversation` table.
...
- HTTP validates Bearer tokens locally via HS256 (`kaji/packages/serve/src/server/deps.py`).
- STT WebSocket auth now uses the same local JWT decode helper in `kaji/packages/serve/src/server/auth_utils.py`.
...
- `kaji/packages/ts/src/providers/` -- `ModelProvider` interface (`base.ts`), `registry.ts` (`registerProvider`/`getProvider`/`clearProviders`), `MockProvider` (drives the full tool loop with no network). Mirrors `kaji/packages/py/src/runtime/providers/base.py`.
...
- `kaji/packages/ts/src/runtime/runtime.ts` -- ports `AgentRuntime.run_turn`: replay -> build messages -> stream from provider -> emit events -> scatter-gather tools -> loop. Plus `CancellationToken` and `buildMessages`. Guards the final completion on truthy text (matches the Python reference).

### 27. Tool-loop glue (DONE)

- The loop in `runtime.ts` turns provider `tool_calls` into `executeTool` (`kaji/packages/ts/src/tools/registry.ts`) calls and emits the full lifecycle (`ToolCallRequested -> Started -> Completed | Failed`), concurrently via `Promise.all`. The real `tool_call_id` is threaded through replay so a real provider can match results to requests.

### 28. Reconcile sync vs async `publish` (DONE -- publish stays async; runtime awaits it)

- `kaji/packages/ts/src/events/bus.ts` -- `EventBus.publish` is `async` (resolved-Promise) with a doc comment locking the decision; the runtime does `await bus.publish(event)`, matching the Python `await self.bus.publish(...)`.
...
- Python source checkout: `uv sync --project kaji/packages/py --extra openai` -> `uv run --project kaji/packages/py kaji init --provider openai`
