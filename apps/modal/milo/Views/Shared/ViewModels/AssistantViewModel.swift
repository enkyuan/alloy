...
#if canImport(StreamingAudio)
import StreamingAudio
#else
// Fallback minimal declarations to satisfy the compiler if StreamingAudioService is missing in this target.
// Remove this block when the real StreamingAudioService is available in the project.
@MainActor
final class StreamingAudioService {
    // Callbacks expected by AssistantViewModel
    var onAudioChunk: ((Data) -> Void)?
    var onError: ((String) -> Void)?

    // State used by AssistantViewModel
    private(set) var isRecording: Bool = false

    // Start streaming audio and invoke onAudioChunk with PCM data
    func startStreaming() async throws {
        // In a fallback stub we just flip the state; real implementation should provide audio.
        isRecording = true
    }

    // Stop streaming audio
    @discardableResult
    func stopStreaming() -> Bool {
