...
def mock_supabase(mock_supabase_auth):
    mock_supabase_auth.get_user = AsyncMock(return_value={"id": "test_user_id"})
...
@pytest.mark.asyncio
async def test_routers_gemini_generate_text(
    async_client: AsyncClient, mock_supabase, mock_gemini_service
):
...
@pytest.mark.asyncio
async def test_routers_gemini_chat_completion(
    async_client: AsyncClient, mock_supabase, mock_gemini_service
):
    headers = {"Authorization": "Bearer token"}
...
@pytest.mark.asyncio
async def test_routers_gemini_generate_stream(
    async_client: AsyncClient, mock_supabase, mock_gemini_service
):
    headers = {"Authorization": "Bearer token"}
