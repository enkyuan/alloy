...
def mock_soniox_service():
    with patch("app.routers.stt.soniox_service") as mock:
...
def mock_websockets_connect():
    with patch("app.routers.stt.websockets.connect") as mock_connect:
        yield mock_connect
...
def test_stt_websocket_flow(test_client: TestClient, mock_supabase_auth, mock_soniox_service, mock_soniox_ws):
    # Mock user auth
    ...
    with test_client.websocket_connect("/api/v1/stt/stream?token=valid_token") as websocket:
...
def test_stt_auth_failure(test_client: TestClient, mock_supabase_auth):
    # Mock auth failure
    mock_supabase_auth.get_user.return_value = None

    with pytest.raises(Exception): 
         with test_client.websocket_connect("/api/v1/stt/stream?token=invalid_token") as websocket:
