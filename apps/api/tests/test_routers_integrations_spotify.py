...
@pytest.mark.asyncio
async def test_routers_integrations_spotify_get_oauth_url(
    async_client: AsyncClient, mock_supabase_auth, mock_settings, mock_redis_global
):
    ...
    response = await async_client.get(
        "/api/v1/integrations/spotify/auth", headers=headers
    )
...
@pytest.mark.asyncio
async def test_routers_integrations_spotify_sync_integration(
    async_client: AsyncClient, mock_supabase_auth, session: Session
):
    ...
    mock_supabase_auth.get_user.return_value = {
        "id": user_id,
        "email": "test@example.com",
        "identities": [{"provider": "spotify", "id": "spotify_user_123"}],
    }
    ...
    response = await async_client.post(
        "/api/v1/integrations/spotify/sync", headers=headers, json=payload
    )
...
@pytest.mark.asyncio
async def test_routers_integrations_spotify_playback_not_connected(
    ...
    headers = {"Authorization": "Bearer test_token"}
    response = await async_client.get(
        "/api/v1/integrations/spotify/playback", headers=headers
    )
...
@pytest.mark.asyncio
async def test_routers_integrations_spotify_playback_success(
    async_client: AsyncClient, mock_supabase_auth, session: Session
):
    ...
    with patch(
        "app.routers.integrations.integrations_spotify.spotify_client"
    ) as mock_spotify:
        mock_spotify.get_valid_token = AsyncMock(return_value="valid_access_token")
        mock_spotify.get_current_playback = AsyncMock(
            return_value={
                "is_playing": True,
        ...
        # Test Play
        response = await async_client.post(
            "/api/v1/integrations/spotify/play?uri=spotify:track:123", headers=headers
        )
        assert response.status_code == 200
        ...
        # Test Pause
        response = await async_client.post(
            "/api/v1/integrations/spotify/pause", headers=headers
        )
        assert response.status_code == 200
        ...
        # Test Next
        response = await async_client.post(
            "/api/v1/integrations/spotify/next", headers=headers
        )
        assert response.status_code == 200
