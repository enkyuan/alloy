...
types: Any = types
ModelContent = cast(Any, getattr(types, "ModelContent", object))
UserContent = cast(Any, getattr(types, "UserContent", object))
Part = cast(Any, getattr(types, "Part", object))
...
def convert_messages_to_gemini(
    ...
    for event in events:
        ...
        if text_events_only and event_type not in (
            AgentResponse,
            UserTranscriptionReceived,
        ):
            continue
        ...
        if event_type in handlers:
            ...
            gemini_messages.append(
                ModelContent(parts=[Part.from_text(text=event.content)])
            )
        elif isinstance(event, UserTranscriptionReceived):
            gemini_messages.append(
                UserContent(parts=[Part.from_text(text=event.content)])
            )
        elif isinstance(event, DTMFInputEvent):
            gemini_messages.append(
                UserContent(parts=[Part.from_text(text=event.button)])
            )
        elif isinstance(event, DTMFOutputEvent):
            gemini_messages.append(
                ModelContent(parts=[Part.from_text(text=event.button)])
            )
        elif isinstance(event, ToolResult):
            # Gemini 400s if a ToolResult is the first message in the context, so don't add it:
            if not gemini_messages:
                logger.debug(
                    f"Skipping orphaned ToolResult at start of context: {event.tool_name}"
                )
                continue
            ...
            gemini_messages.append(ModelContent(parts=[function_response]))

    logger.debug(
        f"Converted {len(events)} messages to {len(gemini_messages)} Gemini messages"
    )
...
def message_to_str(gemini_messages) -> str:
    ...
    for message in gemini_messages:
        ...
        if isinstance(message, UserContent):
            role = "user"
        elif isinstance(message, ModelContent):
            role = "model"
        else:
            role = "unknown"
        ...
        content = (
            serialized_parts[0]
            if len(serialized_parts) == 1
            else json.dumps(serialized_parts, indent=2)
        )

        ans += f"{role}: {content}\n"

        # Extra spacing between user and model turns
        if (
            role == "user"
            and prev_message is not None
            and (isinstance(prev_message, ModelContent))
        ):
            ans += "\n"
...
def serialize_part(part: types.Part) -> str:
    ...
    if hasattr(part, "text") and part.text is not None and part.text.strip() != "":
        ...
        args_str = (
            json.dumps(dict(func_call.args), indent=2) if func_call.args else "{}"
        )
        ...
        response_str = (
            json.dumps(dict(func_resp.response), indent=2)
            if func_resp.response
            else "{}"
        )
