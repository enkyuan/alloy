...
class Turn(BaseModel):
    ...
    telephony_events: list[Union[DTMFOutputEvent, TransferCall, EndCall]] = Field(
    ...
    def to_events(self) -> List[EventInstance]:
        ...
        if self.role == "user":
            ...
            if len(self.text) != 1:
                raise RuntimeError(
                    "Must include exactly one text element for user turn. {len(self.text)=}"
                )
            if self.text:
        ...
        for event in events:
            if isinstance(event, UserTranscriptionReceived):
                ...
                tool_call_map[event.tool_name] = ToolCall(
                    name=event.tool_name, arguments=event.tool_args
                )
    ...
    def is_similar(self, other: "Turn") -> Optional[str]:
        ...
        for self_tool, other_tool in zip(self_tools, other_tools):
            ...
            if self_tool.arguments or other_tool.arguments:
                ...
                if results.is_success is False:
                    return (
                        f"Tool '{self_tool.name}' arguments mismatch: {results.error}"
                    )
