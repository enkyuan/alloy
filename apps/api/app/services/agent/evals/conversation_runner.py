...
class ConversationRunner:
    ...
    def _verify_initial_agent_message(self) -> Optional[List[EventInstance]]:
        ...
        expected_text = first_turn.text
        if isinstance(expected_text, list):
            expected_text = expected_text[0] if expected_text else ""
        if expected_text == self.initial_agent_message:
            return first_turn.to_events()

        results = is_similar_str(self.initial_agent_message, expected_text)
