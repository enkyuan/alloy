...
class VoiceAgentApp:
    ...
    def __init__(
        self,
        call_handler,
        pre_call_handler: Optional[
            Callable[[CallRequest], Awaitable[Optional[PreCallResult]]]
        ] = None,
    ):
        ...
        self.fastapi_app.add_api_route(
            "/chats", self.create_chat_session, methods=["POST"]
        )
    ...
    async def create_chat_session(self, request: Request) -> dict:
        ...
        call_request = CallRequest.model_validate(
            {
                "call_id": body.get("call_id", "unknown"),
                "from": body.get("from_", "unknown"),
                "to": body.get("to", "unknown"),
                "agent_call_id": body.get(
                    "agent_call_id", body.get("call_id", "unknown")
                ),
                "agent": {
        ...
        response: Dict[str, Any] = {"websocket_url": websocket_url}
        if config:
        ...
        call_request = CallRequest.model_validate(
            {
                ...
                "from": query_params.get("from", "unknown"),
