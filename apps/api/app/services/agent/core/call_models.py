...
class PreCallResult(BaseModel):
    ...
    metadata: Dict[str, Any] = Field(
        default_factory=dict, description="Metadata to include with the call"
    )
    config: Dict[str, Any] = Field(
        default_factory=dict, description="Configuration for the call"
    )
...
class AgentConfig(BaseModel):
    ...
    system_prompt: Optional[str] = (
        None  # System prompt to define the agent's role and behavior
    )
    introduction: Optional[str] = (
        None  # Introduction message for the agent to start the call with
    )
