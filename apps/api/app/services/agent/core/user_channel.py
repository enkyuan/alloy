...
def create_user_bridge(harness: "ConversationHarness", authorized_node: str) -> Bridge:
    ...
    async def send_transfer_call(message: Message):
        ...
        timeout = event.timeout_s if event.timeout_s is not None else 30
        return await harness.transfer_call(event.target_phone_number, timeout)
...
def register_observability_event(
    bridge: Bridge, harness: "ConversationHarness", event_type: EventType
):
    """
    ...
    if (
        ...
        and not hasattr(event_type, "to_log_event")
    ):
        raise ValueError(
