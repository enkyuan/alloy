...
class VoiceAgentService:
    ...
    def update_context(
        ...
        if result and result.success and result.data:
            ...
            if "artist" in result.data or "artist_name" in result.data:
                context.last_artist = result.data.get("artist") or result.data.get("artist_name")
            
            if "album" in result.data or "album_name" in result.data:
                context.last_album = result.data.get("album") or result.data.get("album_name")
    ...
    def handle_ambiguity(
        ...
        if intent_type == "play_track":
            ...
            return f"I found multiple matches. Did you mean: {', or '.join(suggestions)}?"
