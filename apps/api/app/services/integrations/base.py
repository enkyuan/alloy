"""Shared HTTP/OAuth base classes for external integrations."""
...
class IntegrationHTTPService:
    """Base reusable HTTP client/service primitives for integrations."""
...
class ExpiringOAuthIntegrationService(IntegrationHTTPService):
    """Base class for OAuth integrations with expiring access tokens."""
    ...
    def _refresh_payload(
        ...
        *,
        ...
        return {
            ...
            "refresh_token": str(integration.refresh_token),
            "client_id": client_id,
    ...
    async def refresh_token(self, integration: Integration, db: AsyncSession) -> str:
        if not integration.refresh_token:
            raise IntegrationAuthError(
                ...
                message=f"{self.SERVICE_NAME} refresh token is missing",
            )
        ...
        if not client_id or not client_secret:
            raise IntegrationAuthError(
                service=self.SERVICE_NAME,
                action="refresh token",
                message=f"{self.SERVICE_NAME} OAuth credentials are not configured",
            )
        ...
        if not isinstance(token_data, dict):
            raise IntegrationAPIError(
                service=self.SERVICE_NAME,
                action="refresh token",
                message=f"{self.SERVICE_NAME} refresh token response payload is invalid",
            )
        ...
        if not access_token:
            raise IntegrationAPIError(
                service=self.SERVICE_NAME,
                action="refresh token",
                message=f"{self.SERVICE_NAME} refresh token response missing access token",
            )

        now = datetime.now(timezone.utc)
        expires_in = int(token_data.get("expires_in", self.DEFAULT_EXPIRES_IN_SECONDS))

        integration.access_token = str(access_token)
        integration.expires_at = now + timedelta(seconds=expires_in)
        integration.updated_at = now
        if "refresh_token" in token_data and token_data.get("refresh_token"):
            integration.refresh_token = str(token_data["refresh_token"])
        await self._commit(db)

        logger.info(
            "Successfully refreshed %s token",
            self.SERVICE_NAME,
    ...
    async def get_valid_token(self, integration: Integration, db: AsyncSession) -> str:
        ...
        if expires_at is not None:
            ...
            if expires_at < datetime.now(timezone.utc) + self.TOKEN_REFRESH_WINDOW:
                logger.info(
                    "%s token is expiring soon, refreshing",
                    self.SERVICE_NAME,
                    extra={"user_id": str(integration.user_id)},
                )
