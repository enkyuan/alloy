        ...
        if data:
            try:
                ...
                context.conversation_history = (
                    data_dict.get("conversation_history") or []
                )
    ...
    def handle_ambiguity(self, options: list[dict], intent: CommandIntent) -> str:
        ...
        if intent_type == "play_track":
            ...
            for i, track in enumerate(options[:3], 1):  # Limit to top 3
                track_name = track.get("name", "")
                artist_name = track.get("artist", "")
                track_suggestions.append(f"{track_name} by {artist_name}")

            return f"I found multiple matches. Did you mean: {', or '.join(track_suggestions)}?"
        ...
        if intent_type == "play_playlist":
            ...
            playlist_suggestions = [p.get("name", "") for p in options[:3]]
            return f"I found multiple playlists: {', or '.join(playlist_suggestions)}?"
        ...
        if intent_type == "play_album":
            ...
            album_suggestions: List[str] = []
            for album in options[:3]:
                album_name = album.get("name", "")
                artist_name = album.get("artist", "")
                album_suggestions.append(f"{album_name} by {artist_name}")

            return f"I found multiple albums: {', or '.join(album_suggestions)}?"
