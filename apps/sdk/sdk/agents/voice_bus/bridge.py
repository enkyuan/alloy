...
from sdk.events.voice_registry import EventInstance, EventsRegistry, EventTypeOrAlias
from sdk.agents.voice_bus.bus import Bus, Message
from sdk.agents.voice_bus.route_builder import RouteBuilder, RouteHandler

if TYPE_CHECKING:
    from sdk.agents.voice_worker.node_base import NodeBase
...
class Bridge:
    ...
    def on(
        ...
        *,
        id: Optional[Union[str, Callable[[str], bool]]] = None,
        source: Optional[Union[str, Callable[[str], bool]]] = None,
        timestamp: Optional[Union[float, Callable[[float], bool]]] = None,
    ...
    async def handle_event(self, message: "Message") -> None:
        ...
        handlers = [
            handler for handler in handlers if handler.should_process_message(message)
        ]
        if not handlers:
            return
        ...
        if tasks:
            ...
            for route_index, (_, result) in enumerate(zip(tasks, results)):  # type: ignore[arg-type]
                if isinstance(result, Exception):
                    logger.error(
                        "Bridge %s: Route %s failed for event %s: %s",
                        self.node_id,
                        route_index,
        ...
        # Cleanup tasks.
        async with self._route_tasks_lock:
            self.active_route_tasks = [
                t for t in self.active_route_tasks if not t.done()
            ]
    ...
    async def _input_router(self) -> None:
        ...
        """
        if self.input_shutdown is None:
            return
        if self.input_source is None or self.bus is None:
            logger.warning(
                "Bridge %s: Input routing missing source or bus", self.node_id
            )
            return
                ...
                if hasattr(self.input_source, "map_to_events"):
                    ...
                    for event in events:
                        await self.bus.broadcast(
                            Message(source=self.node_id, event=event)
                        )
                else:
                    raise ValueError(
                        f"Input source {self.input_source} has no map_to_events method."
                    )
    ...
    def _find_matching_routes(self, event: EventInstance) -> List[RouteHandler]:
        ...
        for pattern, handler in self.routes.items():
            if inspect.isclass(pattern):
                ...
                alias = EventsRegistry.get_alias(type(event))
                if not alias:
                    continue
