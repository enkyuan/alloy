...
def get_tts_provider(provider: str | None = None) -> TTSProvider:
    ...
    Args:
        provider: Override for ``settings.TTS_PROVIDER``. One of ``"none"`` or
            ``"gemini"``. Defaults to the configured value.
    ...
    raise ValueError(f"Unknown TTS_PROVIDER: {name!r} (expected 'none' or 'gemini')")
