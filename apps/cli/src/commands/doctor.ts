...
export function runChecks(o: RunOptions): { checks: Check[]; failed: boolean } {
  ...
  if (langs.includes("ts")) {
    ...
    checks.push({
      ...
      hint: 'Run `bun add "zod@>=4.3 <5"`; Zod is a required kaji peer.',
      severity: "hard",
    });
  ...
  if (langs.includes("python")) {
    ...
    const hasKajiPython =
      /(^|\n)\s*kaji(?:\[[^\]]+\])?\s*(?:@|[<>=~! ]|$)/.test(requirements) ||
      /["']kaji(\[|["'<>=~! ])/.test(pyproject);
    checks.push({
      name: "kaji Python distribution declared",
      ok: hasKajiPython,
      hint: "Use a local Kaji wheel/artifact or source checkout; Python is not published to PyPI.",
