...
describe("doctor.runChecks", () => {
  it("flags missing provider env", () => {
    ...
    writeFileSync(
      ...
      JSON.stringify({
        dependencies: { "kaji-sdk": "0.2.0-beta.6", zod: "4.3.6", openai: "6.42.0" },
      }),
    );
    const out = runChecks({
      cwd: dir,
  ...
  it("passes a no-key mock scaffold with the required TypeScript peers", () => {
    ...
    writeFileSync(
      join(dir, "package.json"),
      JSON.stringify({ dependencies: { "kaji-sdk": "0.2.0-beta.6", zod: "4.3.6" } }),
    );
  ...
  it("flags a missing required Zod peer", () => {
    ...
    writeFileSync(
      join(dir, "package.json"),
      JSON.stringify({ dependencies: { "kaji-sdk": "0.2.0-beta.6" } }),
    );
