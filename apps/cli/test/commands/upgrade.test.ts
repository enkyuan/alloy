...
describe("upgrade.findOutdated", () => {
  it("returns packages where current < latest", async () => {
    ...
    writeFileSync(
      ...
      JSON.stringify({
        dependencies: { kaji: "0.1.0", "@kaji/cli": "0.1.0", other: "1.0.0" },
      }),
    );
    const fakeFetch = vi.fn(async (name: string) => (name === "kaji" ? "0.2.0" : "0.1.0"));
    const out = await findOutdated(dir, fakeFetch);
    expect(out).toEqual([{ name: "kaji", current: "0.1.0", latest: "0.2.0", depType: "prod" }]);
  });
});
